...
import (
	...
	"github.com/EugeneTorap/go-pg-migrate/multistmt"
...
func init() {
	db := Postgres{}
	migrate.Register("postgres", &db)
	migrate.Register("postgresql", &db)
}
...
func WithInstance(instance *sql.DB, config *Config) (migrate.Driver, error) {
	...
	if config.DatabaseName == "" {
		...
		if err := instance.QueryRow(query).Scan(&databaseName); err != nil {
			return nil, &migrate.Error{OrigErr: err, Query: []byte(query)}
		}
...
func (p *Postgres) Open(url string) (migrate.Driver, error) {
...
func (p *Postgres) Lock() error {
	return migrate.CasRestoreOnErr(&p.isLocked, false, true, migrate.ErrLocked, func() error {
		aid, err := migrate.GenerateAdvisoryLockId(p.config.DatabaseName, p.config.migrationsSchemaName, p.config.migrationsTableName)
		if err != nil {
			return err
		}
		...
		if _, err := p.conn.ExecContext(context.Background(), query, aid); err != nil {
			return &migrate.Error{OrigErr: err, Err: "try lock failed", Query: []byte(query)}
		}
...
func (p *Postgres) Unlock() error {
	return migrate.CasRestoreOnErr(&p.isLocked, true, false, migrate.DriverErrNotLocked, func() error {
		aid, err := migrate.GenerateAdvisoryLockId(p.config.DatabaseName, p.config.migrationsSchemaName, p.config.migrationsTableName)
		if err != nil {
			return err
		}
		...
		if _, err := p.conn.ExecContext(context.Background(), query, aid); err != nil {
			return &migrate.Error{OrigErr: err, Query: []byte(query)}
		}
		return nil
	})
}
...
func (p *Postgres) runStatement(statement []byte) error {
	...
	if _, err := p.conn.ExecContext(ctx, query); err != nil {
		if pgErr, ok := err.(*pq.Error); ok {
			...
			return migrate.Error{OrigErr: err, Err: message, Query: statement, Line: line}
		}
		return migrate.Error{OrigErr: err, Err: "migration failed", Query: statement}
	}
	return nil
}
...
func (p *Postgres) SetVersion(version int, dirty bool) error {
	...
	if err != nil {
		return &migrate.Error{OrigErr: err, Err: "transaction start failed"}
	}
	...
	if version >= 0 || (version == migrate.NilVersion && dirty) {
	...
	if err := tx.Commit(); err != nil {
		return &migrate.Error{OrigErr: err, Err: "transaction commit failed"}
	}
...
func (p *Postgres) Version() (version int, dirty bool, err error) {
	...
	case err != nil:
		...
		return 0, false, &migrate.Error{OrigErr: err, Query: []byte(query)}
